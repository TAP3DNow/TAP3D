# the following implementation is based on the official implementation at https://github.com/aliyun/NeWCRFs
'''
...
class NewCRFDepth(nn.Module):
    ...
    def __init__(self, version=None, inv_depth=False, pretrained=None, 
                    frozen_stages=-1, min_depth=0.1, max_depth=100.0,
                    **kwargs):
        super().__init__()
        ...
        ### modified for the thermal dataset, add last conv block to convert the output to 3 channels
    ...
    def forward(self, imgs):
        ...
        if imgs.shape[1] < 3: # the input is not three channels, repeat the last channel to three channels
            imgs = imgs.repeat(1, 3, 1, 1)
        else:
            imgs = imgs[:, -3:, :, :] # use the last three channels
        if imgs.shape[2] != 480 or imgs.shape[3] != 640:
            original_size = imgs.shape[2:4]
            imgs = F.interpolate(imgs, size=(480, 640), mode='bilinear', align_corners=False)
        else:
            original_size = None
            imgs = imgs
        ###### end of modified for the thermal dataset
        ...
        ### modified for the thermal dataset, enabling the depth_3C format
        depth = self.last_conv(depth)
        if original_size is not None:
            depth = F.interpolate(depth, size=original_size, mode='bilinear', align_corners=False)
        ### end of modified for the thermal dataset, enabling the depth_3C format: three channels on: depth, indicator, binary_mask
        return depth
